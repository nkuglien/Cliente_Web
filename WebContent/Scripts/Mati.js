(function ($) {
    ...
    window.confirm = function (mensaje, callback, titulo, valueBoton, tercerBoton, cerrarAntesDeCallBack) {
        cerrarAntesDeCallBack = (cerrarAntesDeCallBack == undefined ) || cerrarAntesDeCallBack == true;
        ...
        var btnAceptar = $("<button type='button' class='btn btn-primary' >" + valueBoton + "</button>");
        btnAceptar.click(function () {
            if (cerrarAntesDeCallBack)
                modal.modal("hide")
...
$(function () {
    ...
    $("#Perfil").click(function () {
        ...
        $.post("/Perfil/GetUsuarioPerfil", function (data) {
            ...
            var buttonClose = $("<button/>").addClass("close")
                .attr({ type: "button", "data-dismiss": "modal", "aria-label": "Cerrar","class" : "close" })
            var spanTitle = $("<span/>").attr("aria-hidden", "true").attr("class" , "glyphicon glyphicon-remove");
            ...
            var spanRol = $("<h3 class='h4TitlePerfilModal'/>").text("Roles").addClass("col-md-12");
            ...
            var labelLectura = $("<label/>").addClass("col-md-4").text("Lectura");
            labelLectura.appendTo(Agrupar5);
            var DivDivLecturaData = $("<div/>").addClass("col-md-2");
            DivDivLecturaData.appendTo(Agrupar5);
            var esLecturaD;
            if (data.Record.esLectura == true)
                esLecturaD = "Si"
            else
                esLecturaD = "No"
            var labelLecturaData = $("<label/>").css("font-weight", "normal").text(esLecturaD)
            labelLecturaData.appendTo(DivDivLecturaData);
           ...
           if (data.Record.esTesoreria == true)
            ...
            var Agrupar8 = $("<div/>").addClass("form-group");
            Agrupar8.appendTo(form);

            var labelaltaProtocolizacion = $("<label/>").addClass("col-md-4").text("Protocolizacion");
            labelaltaProtocolizacion.appendTo(Agrupar8);
            var DivDivaltaProtocolizacionData = $("<div/>").addClass("col-md-2");
            DivDivaltaProtocolizacionData.appendTo(Agrupar8);
            var esProtocolizacionD;
            if (data.Record.esProtocolizacion == true)
                esProtocolizacionD = "Si"
            else
                esProtocolizacionD = "No"
            var labelaltaProtocolizacionData = $("<label/>").css("font-weight", "normal").text(esProtocolizacionD)
            labelaltaProtocolizacionData.appendTo(DivDivaltaProtocolizacionData);
            ...
            var spanNot = $("<h3 class='h4TitlePerfilModal'/>").text("Notificaciones").addClass("col-md-12");
            ...
            ButtonGuardar.addClass("btn btn-primary").text("Guardar");
            ...
            ButtonCancelar.addClass("btn btn-primary").text("Cancelar");
    ...
    $("#Versionado").click(function () {
        $.post("/Versiones/ListadoCompleto", function (data) {
            ...
            $.each(data.Record, function (index, value) {
                var h3 = $("<h4 class='versionTitle'/>").html("Versión N° " + value.Version1 + " <span class='pull-right'>" + value.FechaSubida + "</span>");
                h3.appendTo(divAcordion);
                h3.addClass("manito")
                var divAdentro = $('<div class="contenidoVersiones"/>');
                divAdentro.appendTo(divAcordion);
                $.each(value.ListadoDetalle, function (index, value) {
                    $("<p/>").text(value.detalle).appendTo(divAdentro);
                });
                divAdentro.hide();
                h3.click(function () {
    ...
    $("#Manual").click(function () {
        ...
        alert("Estará disponible en la próxima versión");
